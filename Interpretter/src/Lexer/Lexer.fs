...
module Lexer =
    ...
    let run (input: string) : PositionedToken list =
        let rec scan (input: char list) (pos: int) =
            ...
            | 'a' :: 's' :: 'i' :: 'n' :: tail -> { Token = Asin; Position = pos } :: scan tail (pos + 4)
            | 'a' :: 'c' :: 'o' :: 's' :: tail -> { Token = Acos; Position = pos } :: scan tail (pos + 4)
            | 'a' :: 't' :: 'a' :: 'n' :: tail -> { Token = Atan; Position = pos } :: scan tail (pos + 4)
            | 's' :: 'i' :: 'n' :: 'h' :: tail -> { Token = Sinh; Position = pos } :: scan tail (pos + 4)
            | 'c' :: 'o' :: 's' :: 'h' :: tail -> { Token = Cosh; Position = pos } :: scan tail (pos + 4)
            | 't' :: 'a' :: 'n' :: 'h' :: tail -> { Token = Tanh; Position = pos } :: scan tail (pos + 4)
            | 's' :: 'i' :: 'n' :: tail -> { Token = Sin; Position = pos } :: scan tail (pos + 3)
            | 'c' :: 'o' :: 's' :: tail -> { Token = Cos; Position = pos } :: scan tail (pos + 3)
            | 't' :: 'a' :: 'n' :: tail -> { Token = Tan; Position = pos } :: scan tail (pos + 3)
